...
class MultKAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun='silu', symbolic_enabled=True, affine_trainable=False, grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, seed=1, save_act=True, sparse_init=False, auto_save=True, first_init=True, ckpt_path='./model', state_id=0, round=0, device='cpu'):
        ...
        if auto_save:
            if first_init:
                ...
                print(f"\t\033[30mcheckpoint directory created: {ckpt_path}\033[37m")
                print('\t\033[30msaving model version 0.0\033[37m')
    ...
    def log_history(self, method_name): 
        ...
        if self.auto_save:
            ...
            print('\t\033[30msaving model version '+str(self.round)+'.'+str(self.state_id) + '\033[37m')
    ...
    def plot(self, folder="./figures", beta=3, metric='backward', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0, unnormalised_DataFrame=None):
        '''
        plot KAN
            ...
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l+1]):
                    ...
                    if tick == True:
                        ...
                        x_min_real, x_max_real = x_min, x_max
                        # Only the first layer has labels for the spline plots. The rest is a combination of features
                        if l == 0:
                            if type(unnormalised_DataFrame) == pd.DataFrame :
                                x_min_real, x_max_real = unnormalised_DataFrame[in_vars[i]].min(), unnormalised_DataFrame[in_vars[i]].max()
                            if in_vars != None:
                                plt.xlabel(in_vars[i], fontsize=8, labelpad=-.2)
                            
                        plt.xticks([x_min, (x_min + x_max) / 2, x_max], ['%.2f' % x_min_real, '%.2f' % ((x_min_real + x_max_real) / 2), '%.2f' % x_max_real], fontsize=7)
                        plt.yticks([y_min, y_max], ['%.2f' % y_min, '%.2f' % y_max], fontsize=7)
        ...
        if in_vars != None:
            ...
            for i in range(n):
                if isinstance(in_vars[i], sympy.Expr):
                    plt.gcf().get_axes()[0].text(1 / (2 * (n)) + i / (n), -0.01, f'${latex(in_vars[i])}$', fontsize=40 * scale * varscale, horizontalalignment='center', verticalalignment='top', rotation=90)
                else:
                    plt.gcf().get_axes()[0].text(1 / (2 * (n)) + i / (n), -0.01, in_vars[i], fontsize=40 * scale * varscale, horizontalalignment='center', verticalalignment='top', rotation=90)
        ...
        if out_vars != None:
            ...
            for i in range(n):
                if isinstance(out_vars[i], sympy.Expr):
                    plt.gcf().get_axes()[0].text(1 / (2 * (n)) + i / (n), (y0+z0) * (len(self.width) - 1) + 0.015, f'${latex(out_vars[i])}$', fontsize=40 * scale * varscale, horizontalalignment='center', verticalalignment='bottom')
                else:
                    plt.gcf().get_axes()[0].text(1 / (2 * (n)) + i / (n), (y0+z0) * (len(self.width) - 1) + 0.015, out_vars[i], fontsize=40 * scale * varscale, horizontalalignment='center', verticalalignment='bottom')
        ...
        for _ in pbar:
            ...
            if _ % log == 0:
                if display_metrics == None:
                    pbar.set_description("\033[31m| train_loss:\033[37m %.2e \033[31m| test_loss:\033[37m %.2e \033[31m| reg:\033[37m %.2e \033[31m| \033[37m" % (torch.sqrt(train_loss).cpu().detach().numpy(), torch.sqrt(test_loss).cpu().detach().numpy(), reg_.cpu().detach().numpy()))
                else:
                    string = ''
                    data = ()
